...
class Scrollvideo_Admin {
    ...
    public static function add_columns( $columns ) {
        ...
        foreach ( $columns as $key => $label ) {
            ...
            if ( 'title' === $key ) {
                $new_columns['sv_shortcode'] = __( 'Shortcode', 'scroll-video' );
                $new_columns['sv_video']     = __( 'Video URL', 'scroll-video' );
            }
        }
        return $new_columns;
    }
    ...
    public static function render_columns( $column, $post_id ) {
        switch ( $column ) {
            case 'sv_shortcode':
                echo '<code>[scrollvideo id="' . esc_html( $post_id ) . '"]</code>';
                break;
            case 'sv_video':
    ...
    public static function plugin_action_links( $links ) {
        $manage_link = '<a href="' . esc_url( admin_url( 'edit.php?post_type=scrollvideo' ) ) . '">'
                     . esc_html__( 'Manage', 'scroll-video' )
                     . '</a>';
